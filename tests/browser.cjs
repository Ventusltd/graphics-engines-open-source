...
(async()=>{let browser;const report={checks:[],errors:[]};try{
 ...
 const inv=await page.evaluate(()=>electricalExplorer.blockInventory());assert.equal(inv.modulesPerString,30);assert.ok((await page.locator('#string-stc').textContent()).includes('1377.0 V'));assert.ok((await page.locator('#string-stc').textContent()).includes('1143.0 V'));assert.equal(inv.modules,720);assert.equal(inv.dcHomeCables,48);assert.equal(inv.seriesLinks,696);assert.equal((await state()).mode,'walk');assert.equal(await page.locator('aside').isVisible(),false);
 await page.locator('#direct-top').click();const target=await page.evaluate(()=>electricalExplorer.project(electricalExplorer.getPickTargets()[0].point));await page.mouse.click(target[0],target[1]);await page.waitForSelector('#direct-height');await page.locator('#direct-height').fill('4');await page.locator('#direct-ridge').fill('1');await page.locator('#direct-apply').click();assert.equal((await state()).settings.height,4);assert.equal(await page.evaluate(()=>electricalExplorer.getRidgeGap()),1);assert.equal((await state()).routes.length,48);await page.screenshot({path:path.join(out,'complete-inverter-block.png')});report.checks.push('default 30-series /24-string /48-cable block; walk by default; direct click edits height');await page.locator('#direct-gap').fill('12');await page.locator('#direct-apply').click();assert.equal(await page.evaluate(()=>electricalExplorer.getSpacing()),12);await page.locator('#direct-move').click();const oldPoint=await page.evaluate(()=>electricalExplorer.getPickTargets()[0].point);await page.mouse.move(500,350);await page.mouse.down();await page.mouse.move(540,350);await page.mouse.up();const movedPoint=await page.evaluate(()=>electricalExplorer.getPickTargets()[0].point);assert.ok(movedPoint[0]>oldPoint[0]);await page.locator('#place-table').click();await page.mouse.click(950,500);assert.equal(await page.evaluate(()=>electricalExplorer.blockInventory().strings),26);assert.equal((await state()).routes.length,52);report.checks.push('row gap, mouse table movement, click placement adds two30-module strings/four DC cables');const cableBefore=(await state()).routes[0].length;const inverterTarget=await page.evaluate(()=>electricalExplorer.project(electricalExplorer.getPickTargets().find(t=>t.kind==='inverter').point));await page.mouse.click(inverterTarget[0],inverterTarget[1]);await page.locator('#move-inverter').click();await page.mouse.move(800,350);await page.mouse.down();await page.mouse.move(850,350);await page.mouse.up();assert.notEqual((await state()).routes[0].length,cableBefore);report.checks.push('inverter drag recalculates connected cable lengths');
